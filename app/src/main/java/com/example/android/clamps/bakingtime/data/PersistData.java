package com.example.android.clamps.bakingtime.data;
...
public class PersistData {
        ...
        public static ContentValues[] getRecipeContentValuesFromArray(Context context, ArrayList<Recipe> recipes)
            ...
            for (int i = 0; i < recipes.size(); i++) {
                ...
                mRecipeValues.put(RecipeContract.RecipeEntry.COLUMN_RECIPE_ID, data.getId());
                mRecipeValues.put(RecipeContract.RecipeEntry.COLUMN_RECIPE_NAME, data.getName());
                mRecipeValues.put(RecipeContract.RecipeEntry.COLUMN_RECIPE_SERVINGS, data.getServings());
    ...
    public static ContentValues[] getIngredientsContentValuesFromArray(Context context, ArrayList<Recipe> recipes)
        ...
        for (int i = 0; i < recipes.size(); i++) {
            ...
            for(int j = 0; j < ingredients.size(); j++){
                ...
                mIngredientValues.put(RecipeContract.RecipeEntry.COLUMN_RECIPE_ID, data.getId());
                mIngredientValues.put(RecipeContract.RecipeEntry.COLUMN_INGRDIENT_QUANTITY, current.getQuantity());
                mIngredientValues.put(RecipeContract.RecipeEntry.COLUMN_INGRDIENT_MEASURE, current.getMeasure());
                mIngredientValues.put(RecipeContract.RecipeEntry.COLUMN_INGRDIENT_NAME, current.getIngredient());
        ...
        public static synchronized void cacheIngredientData(final Context context, final ArrayList<Recipe> data) {
            new AsyncTask<Void, Void, Void>() {
                ...
                ingredientContentResolver.delete(
                        RecipeContract.RecipeEntry.INGREDIENTS_CONTENT_URI,
                        null,
                        null);
                /* Insert our new movie data into Movie's ContentProvider */
    ...
    public static ContentValues[] getStepsContentValuesFromArray(Context context, ArrayList<Recipe> recipes)
        ...
        for (int i = 0; i < recipes.size(); i++) {
            ...
            for(int j = 0; j < steps.size(); j++){
                ...
                stepValues.put(RecipeContract.RecipeEntry.COLUMN_RECIPE_ID, data.getId());
                stepValues.put(RecipeContract.RecipeEntry.COLUMN_STEP_ID, current.getId());
                stepValues.put(RecipeContract.RecipeEntry.COLUMN_STEP_SHORT_DESCRIPTION, current.getShortDescription());
                stepValues.put(RecipeContract.RecipeEntry.COLUMN_STEP_DESCRIPTION, current.getDescription());
                stepValues.put(RecipeContract.RecipeEntry.COLUMN_STEP_VIDEO_URL, current.getVideoURL());
                stepValues.put(RecipeContract.RecipeEntry.COLUMN_STEP_THUMBNAIL, current.getThumbnailURL());
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(final Void... unused) {
                ...
                /* Delete old step data */movieContentResolver.delete(
                        RecipeContract.RecipeEntry.STEP_CONTENT_URI,
                        null,
                        null);
                /* Insert our new movie data into Movie's ContentProvider */
                        movieContentResolver.bulkInsert(
